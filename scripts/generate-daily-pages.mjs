...
import { dailyBatch } from "./daily-pages.mjs";

const root = process.cwd();
const siteUrl = "https://beautifulmensclub.com";
const displayDate = "September 1, 2026";
...
upsert("library.html", "BMC DAILY 2026-09-01", `<section class="section" id="september-1-field-manuals"><div class="section-head"><p class="section-tag">SEPTEMBER 1 FIELD MANUALS</p><h2>Three questions. Three useful answers.</h2><p>Choose a clothes steamer, calm a flaky scalp without guesswork, and put critical household records where they can actually help.</p></div><div class="guide-library">${cards}</div></section>`);
...
upsert("index.html", "BMC DAILY 2026-09-01", `<section class="section"><div class="section-head split-head"><div><p class="section-tag">NEW FIELD MANUALS</p><h2>Three better systems.</h2></div><p class="section-intro">Buy the right steamer, stop guessing at scalp flakes, and make critical records usable before an emergency.</p></div><div class="cards">${dailyBatch.pages.map((page, index) => `<article class="card reveal"><div class="card-number">0${index + 1} / ${page.category}</div><div class="card-body"><span>${esc(page.searchIntent.toUpperCase())}</span><h3>${esc(page.title)}</h3><p>${esc(page.description)}</p><a href="${page.slug}">Read the field manual →</a></div></article>`).join("")}</div></section>`);

let library = readFileSync(join(root, "library.html"), "utf8");
library = library.replace(/27 PRACTICAL GUIDES/g, "46 PRACTICAL GUIDES").replace(/Search 27 practical/g, "Search 46 practical").replace(/Twenty-seven useful/g, "Forty-six useful").replace(/>27 guides</g, ">46 guides<").replace(/Updated August 2026/g, "Updated September 2026");
writeFileSync(join(root, "library.html"), library);

let home = readFileSync(join(root, "index.html"), "utf8");
home = home.replace(/Search twenty-seven practical guides/g, "Search forty-six practical guides");
...
writeFileSync(join(root, "publication-manifest.json"), `${JSON.stringify({ generatedAt: dailyBatch.date, site: siteUrl, pages: dailyBatch.pages.map(({ slug, title, category, searchIntent }) => ({ slug, title, category, searchIntent })) }, null, 2)}\n`);
